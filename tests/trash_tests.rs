...
fn test_trash_move_basic() {
    ...
    assert_eq!(trash_entries.len(), 1, "Should have exactly one trash snapshot");
...
fn test_trash_multiple_files() {
    ...
    assert!(!trash_entries.is_empty(), "Should have at least one trash snapshot");
    ...
    assert_eq!(total_files, 2, "Should have 2 total files across all manifests");
}
